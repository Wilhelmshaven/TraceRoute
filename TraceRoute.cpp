...
INT_PTR CALLBACK DlgProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDC_BTN_TRACE:
		{
			...
			{
				//不合法则重新输入
				MessageBox(hdlg, (LPCSTR)"您的输入不合法，请检查！", NULL, MB_OK);
			...
			else
			{
				...
				//启动线程
				sendthread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SendPacket, &sp, 0, NULL);
...
{
	//获得互斥对象
	DWORD WaitMutex = WaitForSingleObject(hMutex, 1);
	...
	while (true)
	{
		if ((res = pcap_next_ex(myDevice.adhandle, &pkt_header, &pkt_data)) > 0)//使用非回调方法捕获数据包
		{
			if (*(WORD *)(pkt_data + 12) == htons(ETH_ARP))//判断ARP包的第13,14位（Type）是否等于0x0806，目的是滤出ARP包			
			{
				...
				{
					//保存获取到的MAC地址
					sprintf(myDevice.gatewayMACStr, "%02X-%02X-%02X-%02X-%02X-%02X", recvARP->ah.source_mac_add[0],
	...
	while (true)
	{
		...
		{			
			if (*(WORD *)(pkt_data + 12) == htons(ETH_IP) && (*(BYTE *)(pkt_data + 23) == ICMP))  //选出发给自己的ICMP的时间超过报文
			{
				...
				//格式化IP以进行比较
				sprintf(dest_ip, "%d.%d.%d.%d", recvICMP->ih.dest_add & 255, recvICMP->ih.dest_add >> 8 & 255,
				...
				if (strcmp(dest_ip, myDevice.ip) == 0)                        //选出自己的ICMP包
				{
					...
					{					
						...
						//格式化IP
						sprintf(hop_ip, "%d.%d.%d.%d", recvICMP->ih.source_add & 255, recvICMP->ih.source_add >> 8 & 255,
							recvICMP->ih.source_add >> 16 & 255, recvICMP->ih.source_add >> 24 & 255);

						//给列表增加项，这里跳数直接用TTL
						AddListViewItems(spara->handle, TTL, delayTime, hop_ip);
						flag = 1;
					}
					...
					if (recvICMP->ICMPHead.type == ICMP_REPLY)
					{
						...
						ReleaseMutex(hMutex);
						WaitForSingleObject(hMutex, INFINITE);
						PostMessage(myhdlg, END_OF_THREAD, 0, 0);//向对话框发送自定义消息：进程已结束
						break;// BREAK ICMP
					}
				}
			}
		...
		if ((flag == 1) || (TimeOut > 1000))
		{
			...
			if (TimeOut > 1000)
			{
				char *out = "*";
				delayTime = -1;
				AddListViewItems(spara->handle, TTL, delayTime, out);
			}
			flag = 0;
			TimeOut = 0;
...
BOOL AddListViewItems(HWND hwndListView, int hop, double time, char *ip_add)
{
	...
	if (time != -1)
	{
		sprintf(ctime, "%6.3f", time);//控制时间部分输出流显示浮点数的数字个数为3
	}
	else ctime = "Time Out!";
